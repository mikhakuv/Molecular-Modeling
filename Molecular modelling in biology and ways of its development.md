...
Электростатический потенциал находится с учётом электрической плотности зарядов в белке $\rho_p(\vec{r})$ и наличия заряженных ионов в растворе(с концентрацией $c_k$ и зарядом $z_k$). Также учитывается изменение диэлектрической проницаемости $E(\vec{r})$ в пространстве. Для нахождения $\phi(\vec{r})$ используется уравнение Пуассона-Больцмана:
$$div(E\cdot grad(\phi)) = -4\pi(\rho_p + \sum_{k=1}^{N} c_k z_k e\ exp(-\frac{z_k e \phi}{RT}))$$
Его можно линеаризовать:
$$div(E\cdot grad(\phi)) = -4\pi(\rho_p + \phi \frac{N_A e^{2} }{kT} \sum_{k=1}^{N} c_k z_k^2 )$$
И решать разностной схемой каждую итерацию моделирования, ведь величины $\rho(\vec{r})$ и $E(\vec{r})$ меняются вместе с движением белков.  
